...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    {
      (w.toLowerCase groupBy identity) map {case (c, list) => (c, list.length)}
    }.toList.sorted
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    wordOccurrences(s.foldRight("")((s1,s2)=> s1+s2))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    val needNoChange: Occurrences = x.filter{case (c,i) => !yMap.exists(p => p._1==c)} //nicht schön aber selten
    val changed: Occurrences = (x diff needNoChange).map(p => (p._1, p._2-yMap.apply(p._1)))
    (needNoChange ::: changed).filter(p => p._2!=0).sorted
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def innerLoop(occ: Occurrences): List[Sentence] = {
      if(occ.isEmpty) List(List())
        ...
        sentence <- innerLoop( subtract(occ,wordOccurrences(word)) )
